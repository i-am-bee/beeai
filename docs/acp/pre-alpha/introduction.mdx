...
<Warning>
  **Fundamental breaking changes are happening now**, affecting protocol,
  transport, and APIs. Consider this version experimental. Join [Alpha
  discussions](/acp/alpha/roadmap#discussions) to help shape it.
</Warning>
...
However, MCP is recognized as an interim solution. Its original design focuses on context sharing, making it an imperfect fit for ACP’s emerging requirements around agent communication. ACP plans to diverge from MCP during its [Alpha](http://localhost:3333/acp/alpha/introduction) phase, addressing this misalignment by establishing a standalone standard specifically optimized for robust agent interactions.